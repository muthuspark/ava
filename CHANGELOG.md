...
- Removed unused `useSpeechRecognition` composable (replaced by `useWhisper`)
- Removed unused Web Speech API type definitions and `src/types/` directory
- Removed `console.error` calls from composables; errors now propagate via reactive `error` refs

### Changed

- Updated README project structure to reflect current codebase
- Replaced duplicated sentence boundary regex in README with reference to source
